...
function  filterTeamS() {
  ...
  let  rows  = document.querySelectorAll("tbody tr");
       ...
       doTheFilter( rows, 0, input); 
}


 function filterTeamBD() {
        let menuItem = document.getElementById("team-filter");

        let word = menuItem.value.toLowerCase();   
        let rows = document.querySelectorAll("tbody tr");
              doTheFilter(rows, 1, word);

    }


  function  doTheFilter(rows, column, word) {
 ...
 let td =  rows[i].getElementsByTagName("td")[column ];
      ...
      if (txtValue.indexOf(word) > -1 || word === "all" ) {
...
//  <th data-columan="points" data-order="desc">Points</th>
$('th[data-column="points"]').on('click', function() {
 ...
 document.getElementById("search").addEventListener("keyup", filterTeamS);
    document.getElementById("team-filter").addEventListener("change", filterTeamBD);
